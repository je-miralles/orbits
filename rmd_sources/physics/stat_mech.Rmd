...
$$
```{r echo=FALSE}
S <- - sum(df2$p * log(df2$p), na.rm = TRUE)
```

We get for this system $S = `r S`$.
...
Narrower distribution smaller entropy, broader distribution bigger entropy.

All states equally probable -> there are no correlations.

When you measure one thing, you modify the probability distribution of the others.

***

**example**

No knowledge, maximum entropy:
...
Unit of entropy is the bit in information theory for a system with two states.

***

**example**

Know the state completely, i.e. $M = 1$, maximum knowledge, zero entropy.

***

**example**

We know only one is tails, rest are heads.
...
**Entropy in continuous phase space**

log of the volume $V_{PH}$ of the continuous region in phase space

$$
S = \log V_{PH}
$$

Now we have a probability density $P$
...
$$
S = - \int P(p, x) \log P(p, x) \mathrm{d p} \mathrm{d x}
$$
...
[Orbits!](../index.html) | [code](https://github.com/je-miralles/orbits/blob/main/rmd_sources/physics/stat_mech.Rmd)
