...
vline_data <- function(XBar_n, quant, s_n) { data.frame(x_int = c(low(XBar_n, quant, s_n), XBar_n, high(XBar_n, quant, s_n))) }
p3 <- ggplot(df3, aes(vote))+ geom_bar() + ggtitle("single year")
```

Let's get away from normals for a second and talk about a different sort of process. Every year an inquiry is made into whether the good citizens of Zilcholand would like to run juice through the water lines instead of water. A yes or no is collected from $N = `r N`$ people.
...
p4 <- ggplot(df4, aes(means)) + theme(legend.position="none") + ggtitle(paste("histogram, means for", as.character(years_less), "years"))
p4 <- p4 + geom_histogram(aes(y=after_stat(density)), binwidth=9/N, na.rm=TRUE)
...
This is a binomial process which can be modeled with some underlying probabilities $P(\text{yes}) = 1 - P(\text{no})$, but over many referenda the mean vote begins to approximate a normal distribution. If we take the average every time, after `r years_less` years we get an estimate for the mean response, $\bar{X}_n =`r XBar_n4`$, and we can establish for the population mean, $\mu$:

- with `r colorize("95% confidence", "red")`, $`r low(XBar_n4, 0.95, s_n4)` \leq \mu \leq `r high(XBar_n4, 0.95, s_n4)`$, or
- with `r colorize("99% confidence", "blue")`, $`r low(XBar_n4, 0.99, s_n4)` \leq \mu \leq `r high(XBar_n4, 0.99, s_n4)`.$

In other words we increase confidence by increasing the spread in our estimate. Nonetheless both of these ranges will usually cover the "true" mean of the simulated Zilcholand we were sampling from, where $P = `r p`$ to vote yes.

The law of large numbers works better with large numbers:
...
p5 <- ggplot(df5, aes(means)) + theme(legend.position="none") + ggtitle(paste("histogram, means for", as.character(years_lots), "years"))
p5 <- p5 + geom_histogram(aes(y=after_stat(density)), binwidth=9/N, na.rm=TRUE)
...
- `r colorize("95% confidence", "red")`, $`r low(XBar_n5, 0.95, s_n5)` \leq \mu \leq `r high(XBar_n5, 0.95, s_n5)`$, or
- `r colorize("99% confidence", "blue")`, $`r low(XBar_n5, 0.99, s_n5)` \leq \mu \leq `r high(XBar_n5, 0.99, s_n5)`.$

Good luck getting $`r years_lots`$ years worth of data!
