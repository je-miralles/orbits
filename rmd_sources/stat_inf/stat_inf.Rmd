...
s_n <- sd(df4$means)
vline_95 <- vline_data(XBar_n, 0.95, s_n)
vline_99 <- vline_data(XBar_n, 0.99, s_n)
p4 <- ggplot(df4, aes(means)) + theme(legend.position="none") + ggtitle(paste("histogram, means for", as.character(years), "years"))
p4 <- p4 + geom_histogram(aes(y=after_stat(density)), binwidth=9/N, na.rm=TRUE)
p4 <- p4 + stat_function(fun=dnorm, args=list(mean=XBar_n, sd=s_n), colour="red")
p4 <- p4 + xlim(XBar_n + c(-1,1) * 3*s_n)
