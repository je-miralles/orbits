...
birth_counts <- read_csv("..\\..\\data\\tools\\flow_maps\\birth_counts.csv", col_types = types)
travel_counts <- read_csv("..\\..\\data\\tools\\flow_maps\\travel_counts.csv", col_types = types)
```
...
n_countries <- nrow(travel_counts)
travel_matrix <- matrix(sample(1:max(travel_counts$Number), n_countries^2, replace = TRUE), nrow = n_countries, ncol = n_countries)
rownames(travel_matrix) <- travel_counts$Country
colnames(travel_matrix) <- travel_counts$Country

# Example usage
#travel_matrix["Albania", "Algeria"]
