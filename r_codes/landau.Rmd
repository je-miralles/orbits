...
$$
\begin{align}
\delta S(q, \dot{q}, t, \eta) &= \frac{\mathrm{d}}{\mathrm{d} \epsilon} \int_{t_1}^{t_2} L(q + \epsilon \eta, \dot{q} + \epsilon \dot{\eta}, t) \mathrm{d}t \Big|_{\epsilon = 0}\\
...
Using [integration by parts](https://en.wikipedia.org/wiki/Integration_by_parts), $\int_{t_1}^{t_2} u(t)\dot{v}(t) \mathrm{d}t = \Big[ u(t) v(t) \Big]_{t_1}^{t_2} - \int_{t_1}^{t_2} \dot{u}(t) v(t) \mathrm{d}t$, with $u = \frac{\partial L}{\partial \dot{q}}$, $\dot{v} = \dot{\eta}$,
