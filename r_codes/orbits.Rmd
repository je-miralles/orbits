...
A very good place to start analyzing any motion is choosing a reference frame and setting up coordinates. Orbital mechanical systems are often considered with respect to the *fixed stars*.

We often try to find a frame that is not accelerating. This is hard! We've had the argument on who's the center of the universe and I hope we're past it; everything turns out to be moving a little bit, even if we can't see it or feel it immediately. But supposing we choose an *inertial frame* like this, we would find that Newton's [laws of motion](https://en.wikipedia.org/wiki/Newton's_laws_of_motion) apply:
...
There are no other forces acting so ${\bf F}_{12} = -{\bf F}_{21}$ by Newtons' third law, and using the [law of gravitation](https://en.wikipedia.org/wiki/Newton%27s_law_of_universal_gravitation) ($F = G \frac{m_1 m_2}{r^2}$) we can find the force acting on the line between the masses.
...
Now applying Newton's second law (${\bf F} = m {\bf \ddot{r}}$) and the definition of $\hat{\bf u}_\mathrm{r}$ we get the accelerations of the two masses, which depend only on the relative position.
...
We can integrate the accelerations to get equations for the velocities and positions of mass $\mathrm{i}$. Keeping in mind that in general the positions, ${\bf r}_\mathrm{i}$, velocities, $\dot{\bf r}_\mathrm{i}$, and accelerations, $\ddot{\bf r}_\mathrm{i}$, depend on time.
...
Together we call these 12 quantities the *state vector* of the system: $S = [{\bf r}_1, {\bf r}_2, {\bf \dot{r}}_1, {\bf \dot{r}}_2]$.
