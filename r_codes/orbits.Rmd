...
get_derived_initial_values <- function(pos_1, pos_2) {
  ...
  init_vals <- data.frame(R0=R0, ddotR0_1=ddotR0_1, ddotR0_2=ddotR0_2)
  init_vals
}
...
get_next_s_vector <- function(s_vector, timestep) {
  # separate the masses
  s_vector_1 <- s_vector[s_vector$obj == 1,]
  s_vector_2 <- s_vector[s_vector$obj == 2,]
  # extract current positions (last row)
  pos_1 <- s_vector_1[nrow(s_vector_1), 3:5]
  pos_2 <- s_vector_2[nrow(s_vector_2), 3:5]
  # extract current velocities (last row)
  vel_1 <- s_vector_1[nrow(s_vector_1), 6:8]
  vel_2 <- s_vector_2[nrow(s_vector_2), 6:8]
  ...
  s_vector[nrow(s_vector) + 1,] <- data.frame(obj = 1, ts = s_vector_1[nrow(s_vector_1), 2] + timestep, new_pos_1, new_vel_1)
  s_vector[nrow(s_vector) + 1,] <- data.frame(obj = 2, ts = s_vector_2[nrow(s_vector_2), 2] + timestep, new_pos_2, new_vel_2)
  s_vector
}
```
