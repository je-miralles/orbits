...
We often try to find a reference frame that is not accelerating. This is hard! Everything turns out to be wagging a little, even what we can't see or feel. But supposing we choose an *inertial frame* like this, we would find that Newton's [laws of motion](https://en.wikipedia.org/wiki/Newton's_laws_of_motion) apply:
...
We first look at the mutual gravitation of two masses, $m_1$ and $m_2$ with positions ${\bf r}_1$ and ${\bf r}_2$ described with their [Cartesian coordinates](https://en.wikipedia.org/wiki/Cartesian_coordinate_system), $x_i$, $y_i$, and $z_i$:
...
get_next_s_vector <- function(s_vector, timestep=1) {
  ...
  s_vector[nrow(s_vector) + 1, 1:8] <- data.frame(n_mass = 1, ts = s_vector[nrow(s_vector) - 1, 2] + timestep, new_pos_1, new_vel_1)
  s_vector[nrow(s_vector) + 1, 1:8] <- data.frame(n_mass = 2, ts = s_vector[nrow(s_vector) - 1, 2] + timestep, new_pos_2, new_vel_2)
  return(s_vector)
}
```
</details>
